...
class ParrotControl(Node):
    def __init__(self):
        ...
        self.publisher_ = self.create_publisher(
            PilotingCommand, "/anafi/drone/command", qos_profile2
        )
        ...
        self.subscription = self.create_subscription(
            Vector3Stamped, "/anafi/drone/speed", self.speed_callback, qos_profile1
        )
        self.subscription = self.create_subscription(
            Odometry, "/anafi/pose/aruco2cam", self.aruco_pose_callback, qos_profile1
        )

        self.timer = self.create_timer(
            0.001, self.publish_message
        )  # Publish at 1000 Hz
    ...
    def aruco_pose_callback(self, msg):
        ...
        print(self.aruco_pose, "Aruco_pose")
    ...
    def publish_message(self):
        ...
        if self.aruco_pose is not None:
            ...
            if self.first_time == 1:
                ...
                self.g = 9.81
                self.tilt_max = 15
                self.U_inv = np.matrix(
                    [
                        [(1 / self.g * self.tilt_max), 0],
